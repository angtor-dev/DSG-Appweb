    ...
    <ul class="dropdown-menu <?= $totalNotificaciones == 0 ? "pb-0" : "" ?>" style="width: 380px;">
        ...
        <li>
            <?php foreach ($notificaciones as $notificacion): ?>
                <button class="dropdown-item d-flex p-0 position-relative" onclick="marcarNotificacionLeida(<?= $notificacion->id ?>)"
                    ...
                    <?php if ($notificacion->getEstado() == EstadoNotif::Pendiente): ?>
                        <div style="position: absolute; top: 8px; right: 8px; width: 10px; height: 10px; background-color: red; border-radius: 50%;"></div>
                    <?php endif ?>
                </button>
            <?php endforeach ?>
        </li>
